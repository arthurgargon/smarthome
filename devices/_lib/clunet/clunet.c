			...
			*/
			...
			case CLUNET_READING_STATE_DATA:    // Чтение всех данных
				...
				if (clunetReadingCurrentBit >= 8){  // Переходим к следующему байту
					...
					clunetReadingCurrentBit = 0;
					if (clunetReadingCurrentByte < CLUNET_READ_BUFFER_SIZE){		//gargon edit
				...
				}else if (clunetReadingCurrentByte >= CLUNET_READ_BUFFER_SIZE){ //Буфер закончился, а сообщение пришло еще не все	->	gargon edit
					clunetReadingState = CLUNET_READING_STATE_IDLE;
					return;
				}
				break;
