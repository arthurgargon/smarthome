...
/*
*	»нициализирует поиск устройств на шине 1-Wire
...
/*
*  «апрос температуры датчика по его описателю OWI_device
...
/*
* «апрос температуры дл€ устройств devices одновременно
*/
...
void heatfloor_systime_request( void (*f)(unsigned char seconds, unsigned char minutes, unsigned char hours, unsigned char day_of_week) ){
	//пришел запрос на получение нового значени€ текущего времени
	//в параметре - функци€ ответа (асинхронно)
			...
			if (m->size >= 1){
				switch(m->data[0]){
					case 1:	//1-wire термометры
							//провер€ем, что запрашиваютс€ все 1-wire термометры
							//и если запрашиваютс€ они - то выполн€ем как дл€ data[0] = 0
